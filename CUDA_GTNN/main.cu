...
void load_matrix_from_txt(const char* filename, std::vector<float>& matrix, int& rows, int& cols) {
    ...
    while (std::getline(file, line)) {
        ...
        // Set or verify number of columns
        if (cols == 0) {
            cols = row.size();
...
void initialize_random_matrix(std::vector<float>& matrix, int rows, int cols) {
    ...
    std::mt19937 gen(rd());
    std::normal_distribution<float> dist(0.0f, 1.0f); // Mean 0, std dev 1
    
    matrix.resize(rows * cols);
    for(size_t i = 0; i < matrix.size(); ++i) {
        matrix[i] = 0.5f * dist(gen); // Multiply by 0.5 like in MATLAB
    }
}
...
int main() {
    ...
    float C       = cfg["C"];
    bool loadFromFile = cfg.value("loadFromFile", false); // New config option
    ...
    // Initialize Q matrix either from file or randomly
    ...
    std::cout << "Simulation complete.\nFinal mean spiking energy = " << finalEnergy << "\n";
